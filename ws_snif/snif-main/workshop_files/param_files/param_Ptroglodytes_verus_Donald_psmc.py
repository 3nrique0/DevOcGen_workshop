...
#for c in [5, 6, 7]:
    ...
    #for w in [0.5, 1]:
        h_inference_parameters = InferenceParameters(
	        data_source = './workshop_files/input_data/Pan_troglodytes_verus_Donald.psmc', #path to the input file
	        source_type = SourceType.PSMC, #type of input (SourceType.PSMC, SourceType.MS or SourceType.JSON)
	        IICR_type = IICRType.Exact, #type of IICR (IICRType.Exact if the input is a PSMC or JSON file, IICRType.T_sim if the input is a ms command that simulates T2, IICRType.Seq_sim if the input is a ms command that simulates genetic sequences)
	        ms_reference_size = None, #ms diploid deme size
	        ms_simulations = None, #number of coalescent times to simulate with ms
...
# 	        bounds_event_times = [(2e4/gtime, 1e5/gtime), (1e5/gtime, 3e5/gtime), (3e5/gtime, 6e5/gtime), (7e5/gtime, 1e6/gtime), (2e6/gtime, 5e6/gtime)],
        ...
        h_settings = Settings(
	        ...
	        custom_filename_tag = 'Ptverus_Donald_frompsmc', #custom tag added to the output name
	        output_directory = './workshop_files/SNIF_results', #path to the output directory
	        default_output_dirname = './SNIF_results_default' #default output directory, created if output_directory is not found
	        )
