...
#for c in [5, 6, 7]:
    ...
    #for w in [0.5, 1]:
        h_inference_parameters = InferenceParameters(
	        data_source = './workshop_files/input_data/Pan_troglodytes_verus_Donald_inferred-model-004_10x100Mb.ms', #path to the input file
	        source_type = SourceType.MSCommand, #type of input (SourceType.PSMC, SourceType.MS or SourceType.JSON)
	        IICR_type = IICRType.Seq_sim, #type of IICR (IICRType.Exact if the input is a PSMC or JSON file, IICRType.T_sim if the input is a ms command that simulates T2, IICRType.Seq_sim if the input is a ms command that simulates genetic sequences)
	        ...
	        psmc_number_of_sequences = 10, #number of ms simulated sequences
	        psmc_length_of_sequences = int(1e8), #length of ms simulated sequences
...
# 	        bounds_event_times = [(2e4/gtime, 1e5/gtime), (1e5/gtime, 3e5/gtime), (3e5/gtime, 6e5/gtime), (7e5/gtime, 1e6/gtime), (2e6/gtime, 5e6/gtime)],
        ...
        h_settings = Settings(
	        ...
	        custom_filename_tag = 'Ptverus_Donald_from10x100Mb', #custom tag added to the output name
